...
# Script to create just the LLM layer

# Exit on error
set -e

STAGE="prod"
LAYER_NAME="${STAGE}-sheet-assist-llm"
...
pip install --no-deps -r layer-llm.txt -t "/tmp/$LAYER_NAME/python"
...
aws lambda publish-layer-version \
    ...
    --description "LLM dependencies layer for SheetAssist" \
...
echo "LLM layer created successfully!"
