...
def construct_status_response(job: dict) -> QueryResponse:
    ...
    if current_status == "completed":
        if output_preferences['type'] == 'online':
            ...
            message += f"Processing complete. Your file should download automatically"
            return QueryResponse(
                status=current_status,
                message=message,
                files=[FileInfo(
        ...
        logger.info(f"Current chunk: {current_chunk}, Chunk status length: {len(chunk_status)}, Chunk status: {chunk_status}")
        chunk_success = "Success" in chunk_status[current_chunk] if current_chunk < len(chunk_status) else False
        logger.info(f"Chunk status: {chunk_status}")
        if output_preferences['type'] == 'online':
